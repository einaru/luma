...
class SearchWorker(QObject):
    ...
    def doWork(self):
        self.success, self.result, self.e = self.lumaConnection.search(
            self.base, self.scope, self.filter,
            self.attrList, self.attrsonly, self.sizelimit
        )
        if self.success:
            self.workDone.emit(
                self.success, self.result, Exception(), self.identStr)
        else:
            self.workDone.emit(
                self.success, self.result, self.e, self.identStr)
